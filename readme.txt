...
main.py: Este es el punto de inicio del programa. Aquí se gestionarán las interacciones del usuario, como la generación de 
		 nuevas claves y el almacenamiento.
	...
	QApplication(sys.argv):
		- QApplication: Es la clase que gestiona el ciclo de eventos de la aplicación y, por lo tanto, es el núcleo de cualquier programa en 
		  PyQt6. Cada aplicación de PyQt necesita una instancia de QApplication para funcionar correctamente.
		- sys.argv: Es una lista que contiene los argumentos de línea de comandos que fueron utilizados para ejecutar el programa.

    Al pasar sys.argv a QApplication, le permites a la aplicación procesar los argumentos de la línea de comandos. Aunque muchas veces no 
    tienes argumentos específicos para procesar, es buena práctica incluirlo por si alguna vez los necesitas.
    Si no hay ningún argumento importante para pasar, podrías simplemente pasar una lista vacía [].
    Esta línea crea una instancia de QApplication, que es esencialmente el motor que hará que tu interfaz gráfica funcione.

	window = PasswordManager():
		- Aquí, se está creando una instancia de la clase PasswordManager, que es tu ventana principal. Esta clase es una subclase de 
		...
		- Esta línea inicializa la ventana principal de la aplicación, que has definido en la clase PasswordManager.

	window.show():
		- Este método es crucial porque le dice a PyQt6 que muestre la ventana en pantalla.
		- Sin show(), la ventana se crea pero no se renderiza en la pantalla, lo que significa que no la verías.
...
gui/interface.py: aquí se concentra todo lo relacionado a la interfaz gráfica de la aplicación.
	Al especificar self como segundo argumento, PyQt6 entiende que este botón debe "vivir" dentro de la ventana principal 
	(PasswordManager). Esto tiene varios efectos:
		- El botón será mostrado en la ventana.
		- La gestión de memoria del botón está vinculada al objeto self. Si la ventana es cerrada o destruida, el botón también 
			lo será automáticamente.
		- La posición y el comportamiento del botón serán relativos a su ventana padre.
    
    QVBoxLayout(): Es un layout (diseño) que organiza los widgets verticalmente, uno debajo del otro. 
...
utils/password_utils.py: Contendrá las funciones para generar contraseñas que cumplan con los requisitos (mayúsculas, números, 
						 caracteres especiales) y realizar sustituciones.
	
	string.ascii_uppercase es una constante en el módulo string que contiene todas las letras mayúsculas del alfabeto en inglés
...
storage/db_manager.py: Aquí se gestionará cómo y dónde se guardan las contraseñas en Sha256
